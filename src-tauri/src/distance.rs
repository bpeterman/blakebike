...
pub fn estimate_distance(samples: &[Telemetry], total_weight_kg: f32) -> DistanceEstimate {
    let mass = f64::from(total_weight_kg).clamp(33.0, 290.0);
    let mut points = Vec::with_capacity(samples.len());
    let mut distance = 0.0;
    let mut previous: Option<(i64, f64)> = None;
    let mut used_trainer = false;
    ...
    for sample in samples {
            ...
            .filter(|speed| speed.is_finite() && *speed >= 0.0)
        {
            ...
            None => (speed_from_power(sample.power_watts, mass), false),
        };
        used_trainer |= trainer_speed;
        used_power |= !trainer_speed;
        max_speed = Some(max_speed.map_or(speed_mps, |current| current.max(speed_mps)));

        if let Some((previous_timestamp, previous_speed)) = previous {
            ...
            if delta_seconds > 0.0 && delta_seconds <= MAX_SAMPLE_GAP_SECONDS {
                distance += (previous_speed + speed_mps) * 0.5 * delta_seconds;
            }
        }
        points.push(DistancePoint {
            speed_mps,
            distance_meters: distance,
        });
        previous = Some((sample.timestamp_ms, speed_mps));
    }

    let source = match (used_trainer, used_power) {
